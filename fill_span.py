...
DEVICE = torch.device('cuda' if torch.cuda.is_available()
                      else 'cpu')  # My envirnment uses CPU

t5_tokenizer = T5Tokenizer.from_pretrained(T5_PATH)
t5_config = T5Config.from_pretrained(T5_PATH)
t5_mlm = T5ForConditionalGeneration.from_pretrained(
    T5_PATH, config=t5_config).to(DEVICE)

# Input text
# A group of people are attending <extra_id_0>.
text = 'A group of people are attending <extra_id_0>. </s>'

encoded = t5_tokenizer.encode_plus(
